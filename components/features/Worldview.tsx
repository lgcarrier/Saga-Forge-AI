...
const Worldview: React.FC<WorldviewProps> = ({ characters, assets, onAssetGenerated }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await generateWorldview(prompt, selectedCharacter);
  ...
  return (
    <div className="h-full flex flex-col space-y-4">
        <GenerationPanel
        title="The Worldview"
        placeholder="e.g., A floating city in the clouds at sunset"
        prompt={prompt}
        setPrompt={setPrompt}
        onGenerate={handleGenerate}
        isLoading={isLoading}
        error={error}
        buttonText="Forge Scene"
        >
            {characters.length > 0 && (
                <div className="flex items-center space-x-3 p-2 bg-brand-dark rounded-lg">
                    <label htmlFor="character-select-worldview" className="font-bold text-brand-parchment flex-shrink-0">Feature Hero:</label>
                    <select
                        id="character-select-worldview"
        ...
        {assets.length > 0 && (
            <div className="flex-grow overflow-y-auto space-y-4 pr-2 bg-brand-dark-secondary p-4 rounded-lg">
                <h3 className="text-xl font-cinzel text-brand-gold sticky top-0 bg-brand-dark-secondary pb-2">Generated Scenes</h3>
                {assets.map(asset => (
                    <div key={asset.id} className="p-4 bg-brand-dark rounded-lg animate-fade-in space-y-4">
                        {asset.imageUrl && <img src={asset.imageUrl} alt="Generated asset" className="rounded-lg w-full object-cover shadow-lg" />}
